...
int main(int argc, char *argv[]) {
    ...
    int maxlen = thread_data_array[0].len;

    for(t = 1; t < NUM_THREADS; t++) {
        if(thread_data_array[t].len > maxlen)
            maxlen = thread_data_array[t].len;
    }
    printf("Showing result for %d threads: %d\n", NUM_THREADS, maxlen);
